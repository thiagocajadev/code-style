...
```json
{
  ...
  "statusHistory": [
    ...
    { "status": "settled", "at": "2026-01-01T10:05:00Z" }
  ],
  "auditLog": [
    { "action": "created", "by": "user-1", "at": "2026-01-01T10:00:00Z" },
    { "action": "settled", "by": "user-2", "at": "2026-01-01T10:05:00Z" }
  ]
}
```
...
```json
{
  ...
  "status": "settled",
  "settledAt": "2026-01-01T10:05:00Z",
  "items": [
    { "productId": "prod-1", "quantity": 2, "unitPrice": 49.9 }
  ],
...
`customerSnapshot` captura os dados relevantes no momento do pedido (desnormalização controlada). Status é um campo simples com `settledAt` ao lado. Histórico de auditoria vai para `order_events` se necessário.
...
const orderSchema = new Schema({
  ...
  status: {
    type: String,
    enum: ["pending", "settled", "shipped", "cancelled"],
    required: true,
  },
...
| Zero ou um | Campo nullable (omitido ou `null`) | `order.settledAt` (null enquanto pendente) |
...
const order = {
  _id: new ObjectId(),
  status: "settled",
  settledAt: new Date("2026-01-01T10:05:00Z"),
  updatedAt: new Date(),
};
...
**Coleção sem schema validator**. Documentos inseridos sem validação de tipo, enum ou campos obrigatórios. Sintoma: campo `status` com valores como `"SETTLED"`, `"settled"`, `"Settled"` convivendo na mesma coleção. Tratamento: `$jsonSchema` validator na coleção; ODM com schema explícito no código.
