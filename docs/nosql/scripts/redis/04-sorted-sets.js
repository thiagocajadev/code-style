...
async function registerGoals(playerId, goals) {
  const newTotal = await client.zIncrBy(
    LEADERBOARD_KEY,
    goals,
    `player:${playerId}`,
  );
...
export async function fetchScorersByGoalRange(minGoals, maxGoals) {
...
async function fetchStandings() {
  const standings = await client.zRangeWithScores(STANDINGS_KEY, 0, -1, {
    REV: true,
  });
