...
`AbstractValidator` valida shape, tipos e constraints — não regras de negócio. Centraliza o
...
Retornar a entidade direta vaza campos internos: `PasswordHash`, `SecurityStamp`, `IsDeleted`.
Usar um `record` de resposta como projeção explícita — nunca a entidade do banco.
